...
public class PaymentController : BaseApiController
{
    ...
    public async Task<ActionResult<IEnumerable<G_PaymentDto>>> Get()
    {
        var Payments = await _unitOfWork.Payments.GetAllAsync();
        return _mapper.Map<List<G_PaymentDto>>(Payments);
    }
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<G_PaymentDto>>> Get([FromQuery] Params PaymentParams)
    {
        ...
        var listaProv = _mapper.Map<List<G_PaymentDto>>(registros);
        return new Pager<G_PaymentDto>(listaProv,totalRegistros,PaymentParams.PageIndex,PaymentParams.PageSize,PaymentParams.Search);
    }
